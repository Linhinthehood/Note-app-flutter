...
class SemanticSearchService {
  ...
  Future<void> _copyDatabaseFromAssets(String dbPath) async {
    try {
      ...
      final vocabResult = await db.rawQuery('SELECT COUNT(*) as count FROM vocabulary');
      ...
      print('Database contains $vocabCount vocabulary terms and $docCount training documents');
      await db.close();
  ...
  String _normalizeVietnamese(String text) {
    final vnMap = {
      'ă': 'a', 'â': 'a', 'á': 'a', 'à': 'a', 'ạ': 'a', 'ả': 'a', 'ã': 'a',
      'ê': 'e', 'é': 'e', 'è': 'e', 'ẹ': 'e', 'ẻ': 'e', 'ẽ': 'e',
      'ô': 'o', 'ơ': 'o', 'ó': 'o', 'ò': 'o', 'ọ': 'o', 'ỏ': 'o', 'õ': 'o',
      'ư': 'u', 'ú': 'u', 'ù': 'u', 'ụ': 'u', 'ủ': 'u', 'ũ': 'u',
      'í': 'i', 'ì': 'i', 'ị': 'i', 'ỉ': 'i', 'ĩ': 'i',
      'ý': 'y', 'ỳ': 'y', 'ỵ': 'y', 'ỷ': 'y', 'ỹ': 'y',
      'đ': 'd'
    };
  ...
  Future<void> _loadIndex() async {
    ...
    print('Loaded ${_idfScores.length} vocabulary terms and ${_docVectors.length} document vectors');
  }

  List<String> _tokenize(String text) {
    final regex = RegExp(r'\b\w+\b');
    return regex.allMatches(text.toLowerCase())
        .map((m) => m.group(0)!)
        .toList();
  }
  ...
  Future<void> indexNotes(List<Note> notes) async {
    ...
    final db = await database;
    final result = await db.rawQuery('SELECT MAX(doc_id) as max_id FROM documents');
    final maxPrebuiltId = (result.first['max_id'] as int?) ?? -1;
    
    print('Adding ${notes.length} user notes to prebuilt database (starting from ID ${maxPrebuiltId + 1})');
  ...
  Future<List<SemanticSearchResult>> search(String query, List<Note> allNotes, {int limit = 20}) async {
    ...
    if (allNotes.isNotEmpty) {
      final db = await database;
      final userResult = await db.rawQuery('SELECT COUNT(*) as count FROM documents WHERE doc_id > 59');
      final hasUserNotes = (userResult.first['count'] as int) >= allNotes.length;
  ...
  Future<List<SemanticSearchResult>> _exactSearch(String query, List<Note> allNotes, int limit) async {
    ...
    for (final entry in _docVectors.entries) {
      ...
      if (similarity > 0.15) { // Higher threshold for exact search
  ...
  Future<List<SemanticSearchResult>> _fuzzySearch(String query, List<Note> allNotes, int limit) async {
    // Find fuzzy matches
    ...
    for (final entry in _docVectors.entries) {
      ...
      if (similarity > 0.05) { // Lower threshold for fuzzy search
  ...
  List<SemanticSearchResult> _stringBasedSearch(String query, List<Note> allNotes, int limit) {
    final results = <SemanticSearchResult>[];
    final queryLower = _normalizeVietnamese(query.toLowerCase());
    
    for (int i = 0; i < allNotes.length; i++) {
      final note = allNotes[i];
      final searchText = _normalizeVietnamese('${note.title} ${note.content} ${note.tags.join(' ')}'.toLowerCase());
